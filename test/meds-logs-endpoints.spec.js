...
describe('Meds Logs Endpoints', () => {
  ...
  context('Given there are meds logs in the database', () => {
    ...
    describe('GET /api/meds_logs', () => {
      it('GET /api/meds_logs responds with 200 and all of the meds logs', () => {
        return supertest(app).get('/api/meds_logs').expect(200);
      });
    });

    describe('POST /api/meds_logs', () => {
      it('POST /api/meds_logs creates a new meds_log, responding with 201 and the newly created meds log', () => {
        const newMedsLog = {
        ...
        return supertest(app)
          ...
          .expect((res) => {
            ...
            expect(res.headers.location).to.eql(`/api/meds_logs/${res.body[0].id}`);
          });
      });
    });

    describe('GET /api/meds_logs/:id', () => {
      it('GET /api/meds_logs/:id responds with 200 and the meds log specified by id', () => {
        const idToGet = 3;
        const specifiedMedsLog = testMedsLogs.filter(
          (medsLog) => medsLog.id === idToGet
        );
        return supertest(app)
          .get(`/api/meds_logs/${idToGet}`)
          .expect(200)
          .then((res) => {
    ...
    describe('DELETE /api/meds_logs/:id', () => {
      it('DELETE /api/meds_logs/:id responds with 204 and removes the meds log', () => {
        const idToRemove = 2;
        const remainingMedsLogs = testMedsLogs.filter(
          (medsLog) => medsLog.id !== idToRemove
        );
        return supertest(app)
          .delete(`/api/meds_logs/${idToRemove}`)
          .expect(204)
          .then((res) =>
            supertest(app).get('/api/meds_logs').expect(remainingMedsLogs)
          );
      });
    });

    describe('PATCH /api/meds_logs/:id', () => {
      it('PATCH /api/meds_logs/:id responds with 201 and the newly edited meds log', () => {
        const idToEdit = 2;
